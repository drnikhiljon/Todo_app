...
with st.form("new_todo_form", clear_on_submit=True): # clear_on_submit to clear form after adding
    ...
    with col_start:
        start_date = st.date_input("Start Date*", value=datetime.date.today(), key="start_date_add")
        start_time_obj = st.time_input("Start Time*", value=datetime.datetime.now().time(), key="start_time_add")
    with col_end:
        end_date = st.date_input("End Date*", value=datetime.date.today(), key="end_date_add")
        end_time_obj = st.time_input("End Time*", value=(datetime.datetime.now() + datetime.timedelta(hours=1)).time(), key="end_time_add") # Default 1 hour later

    details = st.text_area("Task Details*", value="Enter task details here.")
    ...
    if submitted:
        ...
        elif not start_date or not start_time_obj:
            st.error("Start Date and Time are mandatory!")
        elif not end_date or not end_time_obj:
            st.error("End Date and Time are mandatory!")
        elif not details:
            st.error("Task Details are mandatory!")
        else:
...
if not todos_df.empty:
    ...
    with col_filter1:
        search_task_name = st.text_input("Search by Task Name", "", key="search_task_name")
    with col_filter2:
        filter_status = st.selectbox("Filter by Status", ["All", "Pending", "In Progress", "Completed", "Cancelled"], key="filter_status")
    with col_filter3:
        time_filter_option = st.selectbox("Time-Based Filters", ["All", "Overdue", "Upcoming (Next 7 Days)", "Completed Dates"], key="time_filter_option")
    ...
    if not filtered_df.empty:
        # Display filtered DataFrame without the index column
        ...
        # Create display options for the selectbox using the new alphanumeric ID
        ...
        if display_options:
            ...
            # Extract the alphanumeric ID from the selected display string
            # The ID is the first part before " - "
            selected_todo_id = selected_display_option.split(" - ")[0].replace("ID: ", "")
            ...
            if todo_data:
                ...
                with st.form(f"edit_todo_form_{selected_todo_id}"):
                    ...
                    with col_edit_start:
                        ...
                        edit_start_date = todo_dict['start_time'].date() if todo_dict['start_time'] else datetime.date.today()
                        edit_start_time_obj = todo_dict['start_time'].time() if todo_dict['start_time'] else datetime.datetime.now().time()
                    ...
                    with col_edit_end:
                        edit_end_date = todo_dict['end_time'].date() if todo_dict['end_time'] else datetime.date.today()
                        edit_end_time_obj = todo_dict['end_time'].time() if todo_dict['end_time'] else (datetime.datetime.now() + datetime.timedelta(hours=1)).time()
                    ...
                    if todo_dict['media_data']:
                        ...
                        if todo_dict['media_mimetype'] and todo_dict['media_mimetype'].startswith('image/'):
                            st.image(io.BytesIO(todo_dict['media_data']), caption=todo_dict['media_filename'], use_column_width=True)
                        elif todo_dict['media_mimetype'] and todo_dict['media_mimetype'].startswith('video/'):
                    ...
                    if update_button:
                        # Validate all mandatory fields for update
                        if not edit_task_name:
                            st.error("Task Name is mandatory!")
                        elif not edit_start_date or not edit_start_time_obj:
                            st.error("Start Date and Time are mandatory!")
                        elif not edit_end_date or not edit_end_time_obj:
                            st.error("End Date and Time are mandatory!")
                        elif not edit_details:
                            st.error("Task Details are mandatory!")
                        else:
